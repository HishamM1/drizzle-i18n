...
/**
 * Extract a single locale value from a JSON-as-text i18n column (SQLite).
 * Uses json_extract() with bracket-quoted paths for locale tags with special chars.
 *
 * @example
...
export function forLocale(
  ...
  if (opts?.fallback) {
    ...
    const fbExtract = sql`json_extract(${column}, ${sql.raw(`'${fbPath}'`)})`;
...
/**
 * Build a locale-aware SELECT query that LEFT JOINs the translation table (SQLite).
